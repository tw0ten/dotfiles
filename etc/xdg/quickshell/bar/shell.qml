				...
				Repeater {
					...
					BarBlock {
						...
						content: BarText {
							...
							text: `${(workspaces.value[index].is_focused ? workspaces.value[index].name : null) ?? (index <= workspaces.mappedUpTo ? index : `#${String.fromCharCode(0xE000 + index - workspaces.mappedUpTo)}`)}`
						}
					}
				}
				Item {}
